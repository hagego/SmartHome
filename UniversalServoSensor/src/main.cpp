...
void exit() {
  ...
  u_int64_t sleepTime = 60*sleepTimeMinutes*1000000;
...
void mqttCallback(const char topic[], byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,topicSubscribeServoCommand)==0) {
    sprintf(buffer,"recived servo command: %s",payloadString);
    Serial.println(buffer);

    processServoCmd(payloadString);
  }

  // send back confirmation about received message and clear retained message
  sprintf(buffer,"%sReceived",topic);
